		...
		if (m_CurrentData.FrameInfo.AccumulatedFrames > 0)
		{
			...
			if (recordingSource
				...
				if (recordingSource->SourceRect.has_value()
					...
					m_CursorOffsetX = 0 - recordingSource->SourceRect.value().left;
					m_CursorOffsetY = 0 - recordingSource->SourceRect.value().top;
					pProcessedTexture->GetDesc(&frameDesc);
				}
				...
				if (RectWidth(destinationRect) != frameDesc.Width || RectHeight(destinationRect) != frameDesc.Height) {
					...
					m_CursorScaleX = (float)RectWidth(contentRect) / prescaleWidth;
					m_CursorScaleY = (float)RectHeight(contentRect) / prescaleHeight;
				}
